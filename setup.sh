...
# ${FILES_SUBDOMAIN}
FILES_SUBDOMAIN="$(whiptail --inputbox "The subdomain you want static files to be accessed on" 10 75 3>&1 1>&2 2>&3)"
...
# ${ACME_EMAIL}
ACME_EMAIL="$(whiptail --inputbox "The email you want to receive emails from Let's Encrypt to" 10 75 3>&1 1>&2 2>&3)"
# Proxy Password
# ${PASS_HASH}
...
# ${SMTP_SERVER}
SMTP_SERVER="$(whiptail --inputbox "The SMTP server that will be used to send emails with" 10 75 3>&1 1>&2 2>&3)"

# The username to login to SMTP with, usually the email
# ${SMTP_USERNAME}
SMTP_USERNAME="$(whiptail --inputbox "Username for email account" 10 75 3>&1 1>&2 2>&3)"
...
# ${EMAIL_NAME}
EMAIL_NAME="$(whiptail --inputbox "The name that the emails should be sent with" 10 75 3>&1 1>&2 2>&3)"

MASTODON_ADMIN_USER="$(whiptail --inputbox "Username of initial admin account" 10 75 "$EMAIL_NAME" 3>&1 1>&2 2>&3)"
MASTODON_ADMIN_EMAIL="$(whiptail --inputbox "Email of initial admin account" 10 75 "$SMTP_USERNAME" 3>&1 1>&2 2>&3)"
...
MASTODON_SECRET_KEY="$(docker run tootsuite/mastodon:v3.5.3 bundle exec rake secret 2>/dev/null)"
# ${MASTODON_OTP_SECRET}
echo "Generating Mastodon one-time passwords secret..."
MASTODON_OTP_SECRET="$(docker run tootsuite/mastodon:v3.5.3 bundle exec rake secret 2>/dev/null)"
# ${VAPID_KEYS}
echo "Generating Mastodon vapid keys..."
VAPID_KEYS="$(docker run tootsuite/mastodon:v3.5.3 bundle exec rake mastodon:webpush:generate_vapid_key 2>/dev/null)"
...
MINIO_ACCESS=$AWS_ACCESS_KEY_ID
MINIO_SECRET=$AWS_SECRET_ACCESS_KEY
MASTODON_ADMIN_EMAIL=root@pid1.sh
MASTODON_ADMIN_USER=root
...
MASTODON_ADMIN_PASS="$(docker exec -e RAILS_ENV=production mastodonWeb bash -c "sed -i.bak -e '/    - \w\+/d' ~/config/settings.yml && tootctl accounts create $MASTODON_ADMIN_USER --email $MASTODON_ADMIN_EMAIL --confirmed --role admin && cp ~/config/settings.yml.bak ~/config/settings.yml && sed -i -e 's/site_contact_username: .*/site_contact_username: $MASTODON_ADMIN_USER/' -e 's/site_contact_email: .*/site_contact_email: $MASTODON_ADMIN_EMAIL/' ~/config/settings.yml")"
